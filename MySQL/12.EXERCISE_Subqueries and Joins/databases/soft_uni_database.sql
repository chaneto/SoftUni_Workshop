...
FROM `employees` AS E
JOIN `towns` AS T JOIN `addresses` AS A
...
FROM `employees` AS E
JOIN `employees_projects` AS EP JOIN `projects` AS P 
...
#8
SELECT E.`employee_id`, E.`first_name`, CASE WHEN YEAR(P.`start_date`) >= '2005' THEN NULL ELSE P.`name` 
END AS `project_name`
FROM `employees` AS E
JOIN `projects` AS P JOIN `employees_projects` AS EP
ON E.`employee_id` = EP.`employee_id` AND EP.`project_id` = P.`project_id`
