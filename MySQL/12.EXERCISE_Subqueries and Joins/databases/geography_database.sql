...
FROM `countries` AS C
JOIN `continents` AS CONTINENT ON C.`continent_code` = CONTINENT.`continent_code` 
LEFT JOIN `countries_rivers` AS CR ON C.`country_code` = CR.`country_code`
LEFT JOIN `rivers` AS R ON CR.`river_id` = R.`id`
WHERE CONTINENT.`continent_name` = 'Africa' 
GROUP BY C. `country_name`
...
HAVING `currency_usage` > 1
    AND `currency_usage` = (SELECT COUNT(*) AS `currency_usage_2`
    FROM `countries` AS C2
    WHERE C2.`continent_code` = c.`continent_code`
    GROUP BY C2.`currency_code`
    ORDER BY `currency_usage_2` DESC
    LIMIT 1)
ORDER BY C.`continent_code` , C.`continent_code`;

#16
SELECT COUNT(`country_name`) - (SELECT COUNT(distinct(`country_code`) )
...
ON RC.`country_code` = C.`country_code`
JOIN `rivers` AS R
ON R.`id` = RC.`river_id`
GROUP BY C.`country_name`
