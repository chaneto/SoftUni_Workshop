...
#3
DELIMITER $$
CREATE PROCEDURE `usp_get_towns_starting_with`(`nameStartWith` VARCHAR(50))
BEGIN
SELECT `name` AS `town_name`
FROM `towns`
WHERE `name` LIKE CONCAT(`nameStartWith`, '%')
ORDER BY `town_name`;
END$$

CALL `usp_get_towns_starting_with`('N');
...
#6
DELIMITER $$
CREATE PROCEDURE `usp_get_employees_by_salary_level`(`word`  VARCHAR(20))
BEGIN
    IF `word` = 'Low' THEN 
    (SELECT `first_name`, `last_name` FROM `employees` WHERE `salary` < 30000 ORDER BY `first_name` DESC, `last_name` DESC);
   ELSEIF `word` = 'Average' THEN 
   (SELECT `first_name`, `last_name` FROM `employees` WHERE `salary` BETWEEN 30000 AND 50000 ORDER BY `first_name` DESC, `last_name` DESC);
   ELSEIF `word` = 'High' THEN 
   (SELECT `first_name`, `last_name` FROM `employees` WHERE `salary` > 50000 ORDER BY `first_name` DESC, `last_name` DESC);
    END IF;
END$$
DELIMITER ;
