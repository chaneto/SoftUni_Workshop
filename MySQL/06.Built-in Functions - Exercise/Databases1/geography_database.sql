...
(CHAR_LENGTH(REPLACE(`country_name`, 'A', ''))) + 
CHAR_LENGTH(`country_name`) - 
(CHAR_LENGTH(REPLACE(`country_name`, 'a', '')))  >= 3
ORDER BY `iso_code` ASC;
...
#11
SELECT P. `peak_name`, R.`river_name`, CONCAT(LOWER(`peak_name`),
LOWER(SUBSTRING(`river_name`, 2, CHAR_LENGTH(`river_name`)))) AS `mix`
FROM `peaks` AS P, `rivers` AS R
WHERE SUBSTRING(`peak_name`,char_length(`peak_name`),char_length(`peak_name`)) =
