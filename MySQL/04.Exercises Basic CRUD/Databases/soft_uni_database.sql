...
#5
SELECT CONCAT(`first_name`, '.', `last_name`, '@softuni.bg') `full_ email_address`
FROM `employees`;
...
#9
SELECT CONCAT_WS(' ', `first_name`, `middle_name`, `last_name`) `full_name`
FROM `employees`
WHERE `middle_name` IS NOT NULL AND `salary` IN (25000, 14000, 12500, 23600);
...
AS
SELECT CONCAT_WS(' ', `first_name`, `middle_name`, `last_name`) `full_name`, `job_title`
FROM `employees`;
