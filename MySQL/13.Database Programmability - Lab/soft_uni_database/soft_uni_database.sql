...
SET e_count := (SELECT COUNT(T.`name`) AS `count` 
FROM `towns` AS T
JOIN `addresses` AS A
...
SET E.`salary`  = `salary` +  E.`salary` * 0.05
WHERE A.`name` = `department_name`;
END$$
DELIMITER ;
...
BEGIN
IF (`id` < 0 AND `id` > (SELECT COUNT(*) FROM `employees` )) THEN
ROLLBACK;
ELSE
UPDATE `employees`
...
OLD.`last_name`, OLD.`middle_name`, OLD.`job_title`, OLD.`department_id`, OLD.`salary`);
